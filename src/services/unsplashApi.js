...
const imagesPerPage = 16; // Number of images to fetch per page. 
...
export async function fetchImages(query = "", page = 1, { signal } = {}) {
  const normalizedQuery = query.trim(); // Note: "" == falsy 
  /* Prepares the query parameters for the URL */
  ...
  if (normalizedQuery) {  // If there's a search query == true (see above)
